...
const databaseUrl = `https://raw.githubusercontent.com/ponzs212/database/main/token.json`;
  ...
  const thumbnailUrl = "https://files.catbox.moe/qrohd1.jpg"; // FOTO PAS Kirim Bug
...
//-------------- COMMAND BUG --------------//
bot.hears(/^(\/)?(xforce|xcrash|xios)(\s|$)/i, checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
...
bot.hears(/^(\/)?(flowx|xdelay|xbuldo)(\s|$)/i, checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
...
bot.hears(/^(\/)?(xloli|savage)(\s|$)/i, checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
...
cara kirim di command cukup gini
await TrdxtCount(24, target); 

function untuk kirim terus menerus!
